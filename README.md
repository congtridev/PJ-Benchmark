...
Trong lần nộp đầu tiên, chúng tôi sử dụng `std::vector<std::string>` kết hợp QuickSort với phân vùng Lomuto. Cách tiếp cận này có thể bị suy giảm xuống O(N²·L) khi tập dữ liệu chứa nhiều chuỗi trùng lặp, do toán tử `<` của `std::string` phải duyệt lại từ đầu các tiền tố đã khớp ở mỗi lần so sánh. Bên cạnh đó, việc đọc dữ liệu bằng `cin >>` kéo theo hàng trăm nghìn lần cấp phát bộ nhớ động, gây ảnh hưởng đáng kể đến hiệu năng tổng thể.

Để khắc phục, chúng tôi chuyển sang quản lý bộ nhớ thủ công và áp dụng MSD Radix Sort:
